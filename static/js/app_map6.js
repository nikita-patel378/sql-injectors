...
d3.json("/temp-data2", function(response){
    ...
    //         };   
    // };
    //console.log(response.features[0].properties.temperatures[0].temp_max);
    years = [1998, 1999, 2000];
    console.log(years[0]);
    for (let i = 0; i < 52; i++) {
        for (let j = 0; j < 3; j++) {
            console.log(years[j]);
            console.log(response.features[i].properties.temperatures[j].temp_max);
            var style1 = {                
                fillColor: getColor(response.features[i].properties.temperatures[j].temp_max),
                weight: 2,
                opacity: 1,
                color: 'white',
                dashArray: '3',
            ...
            myGeoJSONLayer = L.geoJson(response, {
                style: style1,
                onEachFeature: onEachFeature1
            });
            temp = "temp_" + years[j]
            myGeoJSONLayer.addTo(temp);
            temp.addTo(myMap);
        };
    };
